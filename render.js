    ...
    document.querySelectorAll('.board-column-tasks').forEach(container => {
      new Sortable(container, {
        ...
        onEnd: async (evt) => {
          ...
          // 1. Atualizar colunas e posições localmente para feedback imediato
          const targetTasks = state.tasks.filter(t => t.column_id === toColId && t.id !== taskId);
          targetTasks.sort((a, b) => (a.position || 0) - (b.position || 0));
          targetTasks.splice(evt.newIndex, 0, task);
          ...
          // 2. Se moveu entre colunas, atualizar posições na coluna de origem
          if (fromColId !== toColId) {
            const fromTasks = state.tasks.filter(t => t.column_id === fromColId && t.id !== taskId);
            fromTasks.sort((a, b) => (a.position || 0) - (b.position || 0));
            ...
            if (fromColEl) {
              ...
              if (fromCount) fromCount.textContent = fromTasks.length;
            }
          }
          ...
          if (toColEl) {
            ...
            if (toCount) toCount.textContent = targetTasks.length;
          }
